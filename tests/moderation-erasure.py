...
async def main():
    ...
    try:
        ...
        proc = subprocess.Popen(
            ...
            stdout=(f.BASE / "erasure-challenges.log").open("w"),
        ...
        async with db.transaction():
            ...
            for subtask, owner in [(target, author), (foreign, other)]:
                await db.execute(
                    "INSERT INTO challenges_subtasks(id,task_id,creator,creation_timestamp,xp,coins,ty) VALUES($1,$2,$3,now(),0,0,'question')",
                    subtask,
                    parent,
                    owner,
                )
            await db.execute(
                "SELECT moderation_open($1,$2,'subtask',$3,$4,'own_review',NULL,'{}')", case, moderator, target, author
            )
            command = {
                ...
                "outcome": "provisional",
                "rationale": "Synthetic reason for retained deletion history",
                "ground": "Synthetic independently assessed ground",
                "rule_version": "Synthetic fixture only",
        ...
        token = jwt.encode(
            {"aud": "challenges", "exp": int(time.time()) + 300}, "synthetic-l2-challenges-internal", algorithm="HS256"
        )
        async with httpx.AsyncClient(timeout=15) as c:
            for _ in range(2):
                r = await c.delete(
                    "http://127.0.0.1:55904/_internal/users/" + str(author),
        ...
        assert (
            await db.fetchval("SELECT public_statement::text FROM moderation_decisions WHERE case_id=$1", case)
            == original
        )
        assert not await db.fetchval("SELECT active FROM moderation_holds WHERE case_id=$1", case)
        print(
            "PASS actual internal Challenges T10 deletion and replay204 cross the authenticated erasure guard; own target removed, foreign sibling/shared parent preserved, immutable statement survives and ordinary hold withdrawn",
            flush=True,
        )
        await db.close()
    finally:

...
def normal(value):return re.sub(r'\s+','',plain(value)).replace('ﬁ','fi').replace('ﬂ','fl')
for release in ['2026-09-r1','2026-09-r2']:
 manifest=json.loads((ROOT/f'backend/academy_assets/assets/email/purchase-document-manifest-{release}.json').read_text())
 ...
 for doc in manifest['documents']:
  ...
  pdf=ROOT/doc['pdf'];assert sha(source.read_bytes())==doc['source_sha256'];assert sha(pdf.read_bytes())==doc['pdf_sha256']
  ...
  blocks=re.findall(r'<(h[1-3]|p|li)(?:\s[^>]*)?>(.*?)</\1>',template,re.S);assert len(blocks)==doc['verified_blocks']
  draft=ROOT/'legal/2026-09-complaint/drafts'/pdf.with_suffix('.md').name
  lines=[f'Unveröffentlichte vorgesehene gemeinsame Fassung {document_release}. Kein Wirksamkeitstermin oder Zustimmungsnachweis.','']
  for tag,value in blocks:lines.extend([('#'*int(tag[1])+' ' if tag.startswith('h') else '- ' if tag=='li' else '')+plain(value),''])
 ...
 if release=='2026-09-r2':
  ...
  previous=json.loads((ROOT/'backend/academy_assets/assets/email/purchase-document-manifest-2026-09-r1.json').read_text())['documents'][1]
...
for relative in [old_manifest,'academy_assets/assets/email/agb-2026-09.pdf','academy_assets/assets/email/widerrufsbelehrung-2026-09.pdf']:
 old=subprocess.check_output(['git','show','11386a6d15f94a601b18ea5205998725e228b895:'+relative],cwd=ROOT/'backend')
 assert old==(ROOT/'backend'/relative).read_bytes()
 print('PASS committed final-L1 original bytes unchanged:',relative,sha(old))
source=ROOT/'frontend/pages/docs/privacy.vue';template=re.search(r'<template>(.*?)</template>',source.read_text(),re.S)[1]
blocks=[];headers=0
for tag,body in re.findall(r'<(h[1-3]|p|li|td|th)(?:\s[^>]*)?>(.*?)</\1>',template,re.S):
 value=' '.join(html.unescape(re.sub('<[^>]+>','',body)).split())
 if not value:continue
 if tag=='th':headers+=1
 prefix='#'*int(tag[1])+' ' if tag[0]=='h' and tag!='th' else '- ' if tag=='li' else '**Tabellenspalte:** ' if tag=='th' else ''
 blocks.append(prefix+value)
expected='Unveröffentlichte vorgesehene gemeinsame Fassung 2026-09-r2. Kein Wirksamkeitstermin. Quelle: frontend/pages/docs/privacy.vue; SHA256 '+sha(source.read_bytes())+'\n\n'+'\n\n'.join(blocks)+'\n'
...
assert 'mindestens sechs Kalendermonate' in (ROOT/'backend/academy_persistence/postgres/migrations/2026-09-08-120000_moderation/up.sql').read_text()
print('PASS exact privacy draft: all514 ordered blocks including21 table headers; current source hash, consistent pseudonym wording, actual acknowledgment minimum-calendar wording, publishable templates SECJUR-free')

...
resp = c.post("/auth/users/me/terms", json={"terms_version": "2026-09-r4", "age_confirmed": False})
assert resp.status_code == 412
assert resp.json() == {"detail": "Age not confirmed"}

## both fields are required
for missing in ["terms_version", "age_confirmed"]:
    body = {"terms_version": "2026-09-r4", "age_confirmed": True}
    del body[missing]
...
resp = c.post(
    ...
    json={"terms_version": "2026-09-r4", "age_confirmed": True},
)
...
## success
start = time.time() - 1
resp = c.post("/auth/users/me/terms", json={"terms_version": "2026-09-r4", "age_confirmed": True})
end = time.time() + 1
assert resp.status_code == 200
user["terms_version"] = "2026-09-r4"
...
assert resp.json() == user
assert resp.json()["terms_version"] == "2026-09-r4"
assert c.get("/auth/users/me").json() == user

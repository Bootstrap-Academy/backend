...
async fn setup(pre_wallet: bool) -> PostgresDatabase {
    let root = PathBuf::from(std::env::var("BOOTSTRAP_WALLET_TARGET_FIXTURE").unwrap())
        .canonicalize()
        .unwrap();
    ...
    if std::env::var("BOOTSTRAP_WALLET_RED").as_deref() != Ok("1") {
        let embedded = academy_persistence_postgres::MIGRATIONS
            .iter()
            .find(|m| m.name == FORWARD)
            .expect("wallet forward embedded before reset");
        assert_eq!(embedded.up, std::fs::read_to_string(root.join("source/backend/academy_persistence/postgres/migrations/2026-09-11-050000_wallet_restore_target/up.sql")).unwrap(), "exact current wallet migration embedded before reset");
    } else {
        assert!(
            !academy_persistence_postgres::MIGRATIONS
                .iter()
                .any(|m| m.name == FORWARD),
            "genuine pre-fix binary only"
        );
    }
    if !pre_wallet {
        return common::setup().await;
    }
    let db = common::setup_clean().await;
    let n = academy_persistence_postgres::MIGRATIONS
        .iter()
    ...
    academy_demo::create(
        ...
        academy_persistence_postgres::session::PostgresSessionRepository,
        academy_persistence_postgres::mfa::PostgresMfaRepository,
        academy_persistence_postgres::oauth2::PostgresOAuth2Repository,
    )
    .await
    .unwrap();
    txn.commit().await.unwrap();

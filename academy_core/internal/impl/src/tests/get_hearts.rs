...
use academy_core_internal_contracts::{InternalGetHeartsError, InternalService};
...
async fn ok() {
    ...
    // Act
    let result = sut.get_hearts(&"internal token".into(), FOO.user.id).await;
...
async fn unauthenticated() {
    ...
    assert_matches!(
        result,
        Err(InternalGetHeartsError::Auth(
...
async fn user_not_found() {
    ...
    // Assert
    assert_matches!(result, Err(InternalGetHeartsError::UserNotFound));
}

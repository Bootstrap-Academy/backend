-- Opt-in creation requires current absence; exact legacy receipts and all other branches remain unchanged.
...
BEGIN
 ...
 END IF;
 -- Creation-only callers never follow an active subject that appeared after
 -- preparation. This tests current absence, not unchanged erasure history.
 -- Exact original receipts above remain replayable before this fresh guard.
 IF p_operation='learning_start' AND p_body ? 'expected_no_active_subject' THEN
  IF p_body->'expected_no_active_subject' IS DISTINCT FROM 'true'::jsonb THEN
   RAISE EXCEPTION 'Literal current-absence choice required';
  END IF;
  IF subject_id IS NOT NULL THEN RAISE EXCEPTION 'An active learning subject already exists'; END IF;
 END IF;

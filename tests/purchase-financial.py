...
WORK = Path(tempfile.mkdtemp(prefix="academy-l1-financial-fix-"))
...
provider = http.server.ThreadingHTTPServer(("127.0.0.1", 55623), Provider)
smtp = socketserver.ThreadingTCPServer(("127.0.0.1", 55624), Smtp)
...
try:
    ...
    # Independent additional case: a first original is shared by HTTP and receipt workers.
    controls["render_fail"] = True
    race_oid = new_order(token)
    assert capture(race_oid, token)[0] == 200
    race_payment = settled(race_oid)
    race_number = race_payment["invoice_number"]
    assert sql(f"SELECT count(*) FROM invoice_originals WHERE invoice_number='R{race_number:07}'") == "0"
    controls["render_fail"] = False
    controls["render_marker"] = b"\nINDEPENDENT-FIRST-ORIGINAL-RACE"
    render_before = len(rendered)
    with concurrent.futures.ThreadPoolExecutor(max_workers=7) as pool:
        downloads = [
            pool.submit(req, f"/finance/invoices/{download_token}/{race_number}/invoice.pdf") for _ in range(6)
        ]
        worker = pool.submit(retry)
        download_results = [f.result() for f in downloads]
        worker.result()
    assert all(s == 200 for s, _ in download_results), [s for s, _ in download_results]
    race_original = bytes.fromhex(
        sql(f"SELECT encode(pdf,'hex') FROM invoice_originals WHERE invoice_number='R{race_number:07}'")
    )
    assert all(b == race_original for _, b in download_results)
    assert len(rendered) == render_before + 1, (render_before, len(rendered))
    receipt = json.loads(sql(f"SELECT artifact FROM paypal_receipt_artifacts WHERE order_id='{race_oid}'"))
    assert bytes(receipt["attachments"][0]["bytes"]) == race_original
    (WORK / "invoices" / f"R{race_number:07}.pdf").unlink()
    controls["render_fail"] = True
    controls["render_marker"] = b"\nDIFFERENT-RENDERER-VERSION"
    assert req(f"/finance/invoices/{download_token}/{race_number}/invoice.pdf") == (200, race_original)
    assert len(rendered) == render_before + 1
    controls["render_fail"] = False
    print(
        "PASS INDEPENDENT first-original race: six HTTP downloads plus receipt worker, one render and identical original/receipt bytes; cache loss and renderer drift/outage preserve bytes",
        flush=True,
    )

    # Independent additional case: two incompatible imports for one unknown orphan identity.
    imported_a = WORK / "import-a.pdf"
    imported_b = WORK / "import-b.pdf"
    imported_a.write_bytes(b"%PDF-1.4\nINDEPENDENT-ORPHAN-A\n%%EOF")
    imported_b.write_bytes(b"%PDF-1.4\nINDEPENDENT-ORPHAN-B\n%%EOF")
    money_before = sql("SELECT jsonb_build_object('transactions',count(*),'sum',sum(coins)) FROM transactions")
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        imports = list(
            pool.map(
                lambda file: academy(
                    "admin", "finance", "record-original-invoice", "R9000901", str(file), "independent-synthetic-source"
                ),
                (imported_a, imported_b),
            )
        )
    assert sorted(r.returncode == 0 for r in imports) == [False, True]
    orphan_original = bytes.fromhex(
        sql("SELECT encode(pdf,'hex') FROM invoice_originals WHERE invoice_number='R9000901'")
    )
    assert orphan_original in (imported_a.read_bytes(), imported_b.read_bytes())
    assert sql("SELECT count(*) FROM financial_documents WHERE number='R9000901'") == "0"
    assert sql("SELECT count(*) FROM paypal_coin_orders WHERE invoice_number=9000901") == "0"
    assert req(f"/finance/invoices/{download_token}/9000901/invoice.pdf")[0] == 404
    assert (
        req(f"/finance/invoices/{download_token}/{number}/invoice.pdf", token=other)[0] == 200
    )  # capability itself binds its original owner
    status, other_download_token = req("/finance/token", token=other)
    assert status == 200
    assert req(f"/finance/invoices/{other_download_token}/{number}/invoice.pdf")[0] == 404
    assert sql("SELECT jsonb_build_object('transactions',count(*),'sum',sum(coins)) FROM transactions") == money_before
    print(
        "PASS INDEPENDENT incompatible original-import race: one authoritative winner, no invented owner/register/payment, unknown owner denied and other-owner invoice denied",
        flush=True,
    )

    # Independent concrete counterexample: immutable imported original versus stale legacy cache.
    legacy_cache = WORK / "invoices" / f"R{number:07}.pdf"
    stale_pdf = b"%PDF-1.4\nSTALE-CONFLICTING-LEGACY-CACHE\n%%EOF"
    legacy_cache.write_bytes(stale_pdf)
    legacy_status, legacy_download = req(f"/finance/invoices/{download_token}/{number}/invoice.pdf")
    database_original = bytes.fromhex(
        sql(f"SELECT encode(pdf,'hex') FROM invoice_originals WHERE invoice_number='R{number:07}'")
    )
    observation = {
        "invoice": f"R{number:07}",
        "http_status": legacy_status,
        "database_original_sha256": hashlib.sha256(database_original).hexdigest(),
        "filesystem_cache_sha256": hashlib.sha256(stale_pdf).hexdigest(),
        "download_sha256": hashlib.sha256(legacy_download).hexdigest(),
        "returns_conflicting_cache": legacy_download == stale_pdf,
        "returns_immutable_original": legacy_download == database_original,
        "capture_remains_unknown": sql("SELECT captured_at IS NULL FROM paypal_coin_orders WHERE id='LEGACYT7'") == "t",
        "financial_state_unchanged": sql(
            "SELECT jsonb_build_object('transactions',count(*),'sum',sum(coins)) FROM transactions"
        )
        == money_before,
    }
    (EVIDENCE / "legacy-cache-authority.json").write_text(json.dumps(observation, indent=2))
    assert legacy_status == 200 and legacy_download == database_original and legacy_download != stale_pdf, observation
    assert observation["capture_remains_unknown"] and observation["financial_state_unchanged"]
    legacy_cache.unlink()
    assert req(f"/finance/invoices/{download_token}/{number}/invoice.pdf") == (200, database_original)
    print(
        "PASS actual legacy download preserves immutable imported original despite conflicting cache; removing cache also returns the same original",
        flush=True,
    )

    # Two seconds is solely a prospective synthetic fixture setting.
    CONFIG.write_text(CONFIG.read_text().replace("coins = 3600", "coins = 2"))
    restart_backend()
    late = new_order(token)
    late_snapshot = json.loads(row(late)["snapshot"])
    assert late_snapshot["provision_deadline"]
    time.sleep(2.2)
    assert capture(late, token)[0] == 503
    assert not orders[late]["calls"] and orders[late]["charges"] == ledger(late) == 0
    assert row(late)["last_error"] == "provision_deadline_passed_provider_outcome_unresolved"
    assert json.loads(row(late)["snapshot"]) == late_snapshot
    print("PASS saved deadline prevents new late provider capture without resetting the accepted order", flush=True)

    unknown = new_order(token, "lost_unknown")
    assert capture(unknown, token)[0] == 503 and orders[unknown]["charges"] == 1
    time.sleep(2.2)
    orders[unknown]["get_fail"] = False
    retry()
    unresolved = row(unknown)
    assert unresolved["capture_id"] and unresolved["balance"] is None and ledger(unknown) == 0
    assert orders[unknown]["charges"] == len(orders[unknown]["calls"]) == 1
    assert unresolved["last_error"] == "provision_deadline_passed_requires_review"
    original_contract = json.loads(unresolved["snapshot"])["contract_order_id"]
    assert sql(f"SELECT state FROM purchase_progress WHERE order_id='{original_contract}'") == "review"
    print(
        "PASS actual uncertain capture reconciles after deadline as the original paid unperformed claim; no recapture or invented credit",
        flush=True,
    )

    crossing = new_order(token)
    sql(
        "CREATE FUNCTION l1_late_wallet() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.balance IS NOT NULL AND OLD.balance IS NULL THEN PERFORM pg_sleep(3); END IF; RETURN NEW; END $$; CREATE CONSTRAINT TRIGGER l1_late_wallet AFTER UPDATE ON paypal_payments DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION l1_late_wallet()"
    )
    assert capture(crossing, token)[0] == 200
    sql("DROP TRIGGER l1_late_wallet ON paypal_payments; DROP FUNCTION l1_late_wallet()")
    actual = row(crossing)
    actual_contract = json.loads(actual["snapshot"])["contract_order_id"]
    assert actual["balance"] is not None and ledger(crossing) == orders[crossing]["charges"] == 1
    assert sql(f"SELECT state FROM purchase_progress WHERE order_id='{actual_contract}'") == "review"
    assert (
        sql(
            f"SELECT evidence->>'committed_before_deadline_proven' FROM purchase_provision_observations WHERE order_id='{actual_contract}'"
        )
        == "false"
    )
    assert capture(crossing, token)[0] == 200 and ledger(crossing) == 1
    print(
        "PASS actual wallet COMMIT crossing preserves credit/original ledger while separate timing evidence retains overdue review and claims",
        flush=True,
    )

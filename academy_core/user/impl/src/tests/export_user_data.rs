...
use academy_core_user_contracts::{
    ...
    export::{AccountDataExport, MockUserExportService, ServiceDataExport, UserDataExport},
};
use academy_demo::{
...
fn services() -> MicroserviceExports {
    [("skills".to_owned(), Some(json!({"xp": []})))].into()
}

fn exported_services() -> BTreeMap<String, ServiceDataExport> {
    [(
        "skills".to_owned(),
        ServiceDataExport::Data(json!({"xp": []})),
    )]
    .into()
}

/// One service that answered and one that could not be read.
fn incomplete_services() -> MicroserviceExports {
    [
        ("skills".to_owned(), Some(json!({"xp": []}))),
        ("events".to_owned(), None),
    ]
    .into()
}

fn exported_incomplete_services() -> BTreeMap<String, ServiceDataExport> {
    [
        (
            "skills".to_owned(),
            ServiceDataExport::Data(json!({"xp": []})),
        ),
        ("events".to_owned(), ServiceDataExport::Unavailable),
    ]
    .into()
}
...
/// An export that is missing the data of a microservice names the missing
/// service and is not marked as complete.
#[tokio::test]
async fn microservice_unavailable() {
    // Arrange
    let auth = MockAuthService::new().with_authenticate(Some((FOO.user.clone(), FOO_1.clone())));
    ...
    let microservices_api =
        MockMicroservicesApiService::new().with_export_user(FOO.user.id, incomplete_services());
    ...
    assert_eq!(
        ...
        UserDataExport {
            account: account(),
            services: exported_incomplete_services()
        }
    );
    assert!(!result.is_complete());
}

...
pub struct ApiUserDataExport {
    /// Whether every microservice could be read, so that this document is
    ...
    pub services: BTreeMap<String, ApiServiceExport>,
}

/// The data a single microservice stores about a user.
#[derive(Debug, Clone, Serialize, JsonSchema)]
pub struct ApiServiceExport {
    /// Whether the data of this service could be read
    pub available: bool,
    /// The data of this service, null if it could not be read
...
#[derive(Debug, Clone, Serialize, JsonSchema)]
pub struct ApiExportHearts {
    /// Number of hearts the user currently has
    pub hearts: u64,
    /// Point in time at which the hearts were last refilled
    pub last_refill: ApiTimestamp,
}
    ...
    fn from(value: UserDataExport) -> Self {
        Self {
            ...
            services: value
                .services
                .into_iter()
                .map(|(service, export)| (service, export.into()))
                .collect(),
        }
    }
}

impl From<ServiceDataExport> for ApiServiceExport {
    fn from(value: ServiceDataExport) -> Self {
        match value {
            ServiceDataExport::Data(data) => Self {
                available: true,
                data: Some(data),
            },
            ServiceDataExport::Unavailable => Self {
                available: false,
                data: None,
            },
        }
...
impl From<Hearts> for ApiExportHearts {
    fn from(value: Hearts) -> Self {
        Self {
            hearts: value.hearts,
            last_refill: value.last_refill.into(),
        }
    }
}

    ...
    CoinRepo: CoinRepository<Db::Transaction>,
{
    ...
    async fn export_user_data(
        ...
        // A microservice that cannot be read is recorded as unavailable
        // instead of failing the request, so that the user still receives the
        // rest of their data. The export names the missing part, so it is
        // never handed out as if it were complete.
        let services = self
            .microservices_api
            .export_user(user_id)
            .await
            .into_iter()
            .map(|(service, data)| (service, data.into()))
            .collect();

        let export = UserDataExport { account, services };

        // Deliberately without the id of the user: the export is about their
        // personal data and does not belong in the logs.
        info!(
            services = export.services.len(),
            complete = export.is_complete(),
            "Exported the data of a user"
        );

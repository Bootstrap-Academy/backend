...
The record is written inside the transaction that deletes the account, because it carries everything a later refund needs; its pdf is rendered afterwards, so that no database transaction is held open across the http call to the render daemon.
A render daemon that is unavailable therefore only costs the pdf, never the deletion, and the failure is logged with the document number alone.
...
- `academy task prune-database`: Deletes sessions that have not been refreshed within `session.refresh_token_ttl`, administrative audit log entries older than twelve months (`academy_models::admin_audit::ADMIN_AUDIT_LOG_RETENTION_MONTHS`) and cancellation and withdrawal declarations that are older than `contract.retention_years` years, counted from the end of the calendar year in which they were received (§ 195, § 199 Abs. 1 BGB).
